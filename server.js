...
// Osvježi preslike - spremanje na datotečni sustav
app.get('/refresh-backups', requiresAuth(), async (req, res) => {
    try {
        const upit = `SELECT k.*, z.naziv_zanra AS zanr FROM knjige k 
                      LEFT JOIN zanrovi z ON k.id = z.knjiga_id`;
        const rez = await pool.query(upit);
        const podaci = rez.rows;

        // JSON
        fs.writeFileSync(path.join(__dirname, 'public', 'knjiznica.json'), JSON.stringify(podaci, null, 2));

        // CSV
        let csv = 'id,naziv,autor,godina_izdanja,izdavac,mjesto_izdanja,isbn,broj_stranica,jezik,zanr,broj_primjerka,dostupnost\n';
        podaci.forEach(r => { csv += `${r.id},${r.naziv},${r.autor},${r.godina_izdanja},${r.izdavac},${r.mjesto_izdanja},${r.isbn},${r.broj_stranica},${r.jezik},${r.zanr},${r.broj_primjerka},${r.dostupnost}\n`; });
        fs.writeFileSync(path.join(__dirname, 'public', 'knjiznica.csv'), csv);

        res.send("Izvoz podataka iz baze podataka te spremanje na datotečni sustav uspješno obavljeno.");
    } catch (err) {
